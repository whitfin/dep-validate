 ...
 *
 * This will return anOobject containing two keys against any
...
function log(results, stream) {
  ...
  if (dependencyErrors) {
    stream.write(dependencyErrors + '\n');
  }

  if (devDependencyErrors) {
    stream.write(devDependencyErrors + '\n');
  }
}
...
function _processDeps(deps, enforce, opts) {
  ...
  Object.keys(deps).forEach(function (dependencyName) {
    ...
    if (/[0-9]/.test(firstCharacter)) {
      return allowHardcoded || emitError(dependencyName, dependencyVersion, enforce);
    }
